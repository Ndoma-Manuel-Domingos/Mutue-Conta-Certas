...
<html lang="pt">
    ...
    <head>
        ...
        <title>LISTA ESTUDANTES COM DESCONTOS</title>
    ...
    <table class="table table-stripeds" style="">
        <thead>
            ...
            <tr style="background-color: #3F51B5;color: #ffffff;">
                <th colspan="9" style="padding: 5px;">LISTA ESTUDANTES COM DESCONTOS</th>
            </tr>
                        
            <tr style="background-color: #a1a4b9;color: #ffffff;">
                <th colspan="3" style="padding: 5px;">Ano Lectivo:</th>
                <th colspan="6" style="padding: 5px;">{{ $ano ? $ano->Designacao : 'Todos' }}</th>
            </tr>
            
            <tr style="background-color: #a1a4b9;color: #ffffff;">
                <th colspan="3" style="padding: 5px;">Semestre:</th>
                <th colspan="6" style="padding: 5px;">{{ $semestre ? $semestre->Designacao : 'Todos' }}</th>
            </tr>            
            
            <tr style="background-color: #a1a4b9;color: #ffffff;">
                <th colspan="3" style="padding: 5px;">Tipo Desconto:</th>
                <th colspan="6" style="padding: 5px;">{{ $desconto ? $desconto->descricao : 'Todos' }}</th>
            </tr>
           
            <tr style="background-color: #a1a4b9;color: #ffffff;">
                <th colspan="3" style="padding: 5px;">Total de DE REGISTOS:</th>
                <th colspan="6" style="padding: 5px;">{{ count($listarAlunoDesconto) }}</th>
            </tr>
                
            <tr style="background-color: #3F51B5;color: #ffffff">
                <th style="text-align:left" width="2px">Nº</th>
                <th style="text-align: center;padding: 4px 0" width="150px">Matrícula</th>
                ...
                <th style="text-align: center;padding: 4px 0">Afectação</th>
                <th style="text-align: center;padding: 4px 0">Período</th>
        ...
        <tbody>
            @foreach ($listarAlunoDesconto as $item)
                <tr>
                    <td style="text-align:left">{{$loop->iteration}}</td>
                    <td style="padding: 5px;text-align: center">{{ $item->matricula }}</td>
                    <td style="text-align: center">{{ $item->Nome_Completo }}</td>
                    <td style="text-align: center">{{ $item->NomeInstituicao }}</td>
                    <td style="text-align: center">{{ $item->tipoDesconto }}</td>
                    <td style="text-align: center">{{ $item->afectacao }}</td>
