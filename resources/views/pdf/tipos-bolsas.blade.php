...
<html lang="pt">
    <head>
        ...
        <title>LISTA DE TIPOS DE BOLSAS</title>
...
<table>
    <tr style="background-color: #e3e7e8; color: #000000;">
        <th colspan="6" style="text-align: left; padding: 10px; text-transform: uppercase;">
            LISTA DAS INSTITUIÇÕES TIPO DE BOLSAS
        </th>
    </tr>
</table>
<main>
    ...
    <table class="table table-stripeds" style="">
        <thead>
            <tr style="background-color: #152ea2;color: #e3e7e8">
                ...
                <th style="text-align: center;padding: 1px 0">Codigo</th>
                <th style="text-align: center;padding: 1px 0">Designação</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($tiposBolsas as $item)
                <tr>
                    <td style="text-align:left">{{$loop->iteration}}</td>
                    <td style="padding: 5px;text-align: center">{{ $item->codigo}}</td>
                    <td style="padding: 5px;text-align:center">{{ $item->designacao ?? '...'}}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
