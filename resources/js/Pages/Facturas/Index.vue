  ...
  <MainLayouts>
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-info">LISTAGEM DE FACTURAS</h1>
          </div>
          <div class="col-sm-6">
          ...
          <div class="col-12 col-md-12">
            <div class="card">
              <div class="card-header">
                <a href="/facturas/create" class="btn btn-info btn-sm"> <i class="fas fa-plus"></i> REGISTRAR FACTURAS</a>
                <a @click="imprimirPlano()" class="btn btn-sm mx-1 btn-danger float-right"> <i class="fas fa-file-pdf"></i> PDF</a>
                <a href="" class="btn btn-sm mx-1 btn-success float-right" @click="ExportarExcelMovimento()"><i class="fas fa-file-excel"></i> Excel</a>
              </div>
              <div class="card-body">
                <table class="table table-bordered table-hover" id="tabela_de_movimentos">
                  ...
                  <tbody>
                    ...
                    <tr v-for="item in facturas" :key="item">
                      <td>{{ item.id }}</td>
                      <td>{{ item.data_factura }}</td>
                      <td>{{ item.data_vencimento }}</td>
                      <td>{{ item.exercicio.designacao }}</td>
                      <td>{{ item.periodo.designacao }}</td>
                      
                      <td>{{ item.cliente.numero }} - {{ item.cliente.designacao }}</td>
                      <td>{{ item.fornecedor.numero }} - {{ item.fornecedor.designacao }}</td>
                      
                      <td><span class="badge" :class="{
                        'bg-info': item.status === 'Pendente',
                        'bg-success': item.status === 'Pago',
                        'bg-danger': item.status !== 'Pendente' && item.status !== 'Pago'
                      }">{{ item.status }}</span></td>
                      <td class="text-right"><strong>{{ formatarValorMonetario(item.valor_total) }}</strong></td>
                      <td>
                        <div class="float-right">
                          <a :href="`/facturas/${item.id}/edit`" class="btn btn-sm btn-success mx-1"><i class="fas fa-edit"></i> Editar</a>
                        </div>
                      </td>
                    </tr>
