...
class EstudantePropinaPagaTotaisExport extends DefaultValueBinder implements FromCollection, WithMapping, WithTitle, WithHeadings, WithDrawings, WithStyles, WithCustomStartCell, WithCustomValueBinder, ShouldAutoSize
{
    use TraitHelpers, Exportable;

    public $searchAnoLectivo , $searchMes, $titulo , $searchFaculdade , $searchCurso , $searchTurno, $totais;
    ...
    public function __construct($request)
    {
        ...
        $this->titulo = "TOTAL DE ESTUDANTES COM MENSALIDADES PAGAS";
    }
    ...
    */
    public function collection()
    {
        ...
        if($this->searchCurso == "0"){
            $this->searchCurso = "";
        }else{
            $this->searchCurso = $this->searchCurso;
        }
        ...
        if ($anoSelecionado >= 2 and $anoSelecionado <= 15) {
            ...
            $query->select(
                ...
                'tb_matriculas.Codigo_Curso AS curso_id',
            );
        } else {
        ...
        $data['cursos'] = $query->distinct('tb_cursos.Codigo')
        ->select(
            'tb_cursos.Codigo AS curso_id',
            'tb_cursos.Designacao AS curso',
            \DB::raw('COUNT(DISTINCT tb_matriculas.Codigo) AS totalEstudantes')
        )
        // ->whereIn('tb_cursos.Codigo', [19, 30])
        ->groupBy('tb_cursos.Codigo')
        ->get();     
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $faculdade = Faculdade::find($this->searchFaculdade);
        $curso = Curso::find($this->searchCurso);
        $turno = Turno::find($this->searchTurno);
        $mes = MesTemp::find($this->searchMes);
        $ano = AnoLectivo::find($anoSelecionado);
    
        $sheet->setCellValue('A7', strtoupper($this->titulo));
        $sheet->setCellValue('B4', 'FACULDADE: ');
        $sheet->setCellValue('C4', $faculdade ? $faculdade->designacao : 'Todos');
        $sheet->setCellValue('B5', 'CURSO ');
        $sheet->setCellValue('C5', $curso ? $curso->Designacao : 'Todos');
        $sheet->setCellValue('B6', 'TURNO: ');
        $sheet->setCellValue('C6', $turno ?  $turno->Designacao : 'Todos');
        $sheet->setCellValue('B7', 'PARCELA: ');
        $sheet->setCellValue('C7', $mes ? $mes->designacao : 'Todos');
        $sheet->setCellValue('B8', 'TOTAL REGISTROS: ');
        $sheet->setCellValue('C8', "6291" );
